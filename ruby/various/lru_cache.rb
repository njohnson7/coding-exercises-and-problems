...
def lru_cache(arr)
  cache = []
  arr.each do |letter|
    cache.delete(letter) if cache.include?(letter)
    cache.shift if cache.size >= 5
    cache << letter
  end
  cache.join('-')
end

p lru_cache(["A", "B", "C", "D", "A", "E", "D", "Z"]) # 'C-A-E-D-Z'
p lru_cache(["A", "B", "A", "C", "A", "B"]) # "C-A-B"
p lru_cache(["A", "B", "C", "D", "E", "D", "Q", "Z", "C"]) # "E-D-Q-Z-C"
